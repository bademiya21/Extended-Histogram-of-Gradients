...
RHOGDense::RHOGDense(
		...
		const DescNormalizer<RealType>* n1
        ):
    cellsize_(cellsize_), 
    numcell_(numcell_), 
    stride_(stride_),
	...
	h_extent1( extent_[0], extent_[1], 360.0),
    h_bandwidth1( cellsize_[0], cellsize_[1], 360.0/orientbin_),
	
    tmag_(extent_),
    hist_(0.0,h_extent1,h_bandwidth1,blitz::TinyVector<bool,3>(false,false,true)),
	hist1_(0.0,h_extent1,h_bandwidth1)
{
    if (wtscale_ > 1e-3) {
...
{// {{{
	...
	//Transforming direction histogram to difference of direction(orientation) histogram
	for (int i=0; i<numcell_[0]; ++i){
		for (int j=0; j<numcell_[1]; ++j){
			for (int k=0; k<(orientbin_/2); ++k){
				TinyVector<RealType,3> ind ( i, j, k );
				TinyVector<RealType,3> ind1 ( i, j, k+(orientbin_/2) );
				hist1_.addbin(ind,(hist_[ind] + hist_[ind1]));
				hist1_.addbin(ind1,(hist_[ind] - hist_[ind1]));			
			}
		}
	}
