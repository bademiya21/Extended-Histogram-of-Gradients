...
{// {{{
    ...
    if (memorylimit) {
        ...
        unsigned int maxdescriptor=memorylimit*1024/descsize1;
        if (poscases)
...
{ // {{{ 
            ...
            f!= inlist.end();++f)
    {
        ...
        if (nopyramid) {
            ...
            DetectInfo r = bound(classifier(desc.data()),1, tl, windesc->extent(),hardtest);
			//printf("Score: %f\n",r.score);
			//avg += r.score;
            r.lbound -=toadd;
...
LinearClassify::LinearClassify(std::string& modelfile, const int verbose) 
    :
    linearwt_(0),  linearbias_(0), kernel_type(0), poly_degree(0), rbf_gamma(0), coef_lin(0), coef_const(0),
		totwords(0), sv_num(0), weight(0), sup_vec(0), sup_vec_norm(0)
{// {{{
    if (verbose > 2) 
    ...
    if(kernel_type == 0) { /* linear kernel */
		...
		//sup_vec_norm = new double[sv_num-1];
		for (int i=0; i<sv_num-1; i++){
			...
			for(int j=0; j<totwords; j++){
				...
				//sup_vec_norm[i] += sup_vec[i*totwords+j]*sup_vec[i*totwords+j];
				//if (i==0)
					//printf("%e\t",sup_vec[i*totwords+j]);
			}
			//printf("Norm of Support Vector %ld: %e \n",i+1,sup_vec_norm[i]);
			//if (i==0)
				//printf("\n");
			long ll;
			fread(&(ll), sizeof(long),1,modelfl);
			char *comment = new char[ll];
...
/*LinearClassify::LinearClassify(const LinearClassify& o) :
    linearbias_(o.linearbias_), kernel_type(o.kernel_type), poly_degree(o.poly_degree), rbf_gamma(o.rbf_gamma), coef_lin(o.coef_lin),
	coef_const(o.coef_const), totwords(o.totwords), sv_num(o.sv_num)
{
	if (kernel_type == 0){
		linearwt_ = new double[totwords];
		std::copy(o.linearwt_, o.linearwt_+o.totwords, linearwt_);
	}
	else {
		weight = new double[sv_num-1];
		sup_vec = new double[(sv_num-1)*totwords];
		std::copy(o.weight, o.weight+o.sv_num-1, weight);
		std::copy(o.sup_vec, o.sup_vec+(o.sv_num-1)*o.totwords, sup_vec);
	}	
}
...
LinearClassify& LinearClassify::operator=(const LinearClassify& o) 
{
    if (&o != this) {
		if (o.kernel_type == 0){
			...
			poly_degree=o.poly_degree;
			rbf_gamma=o.rbf_gamma;
			coef_lin=o.coef_lin;
			coef_const=o.coef_const;
			totwords=o.totwords;
			sv_num=o.sv_num;
			linearbias_=o.linearbias_;
	...
	switch(kernel_type){
		...
		case 2: /*radial basis function*/
			...
			for (int j=sv_num-1; j--; ){
				...
				//printf("Weight*Exp: %e\n",weight[j]*exp(-rbf_gamma*(sum3-2*sum1+sup_vec_norm[j])));
			}
			//printf("Sum: %e\n",sum - linearbias_);
			return sum - linearbias_;
			break;
		...
		default: /*custom-kernel & unknown option*/
			throw Exception("LinearClassify::operator()","Unknown kernel function\n");
			exit(1);
			break;
	}
}
