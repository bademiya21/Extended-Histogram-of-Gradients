 ...
 * 
 *        Version:  1.0
 *        Created:  Sunday 03 June 2007 12:24:12  IST IST
 *       Revision:  none
 *       Compiler:  gcc
 * 
...
struct RHOGDenseParam {
    ...
    NormalizerFlags                     norm2use, norm2use_exhog;
    ...
    RHOGDenseParam():
        ...
        gscale(0), epsilon(1), epsilon1(0.001), maxvalue(0.2), maxvalue1(0.2),
        preprocessing2use(RGB_Sqrt_Grad), norm2use(NormL2Hys), norm2use_exhog(NoNorm)
    { }
...
class LinearClassify {
    ...
    double operator()(const double* desc, const bool use_bias=false) const ;

    float operator()(const float* desc, const bool use_bias=false) const ;

    ~LinearClassify() {
        	delete[] posavg;
		delete[] negavg;
		for (int i=totwords;i--;){
			delete[] poscovmatinv[i];
			delete[] negcovmatinv[i];
			delete[] transformvec[i];
		}
		delete[] poscovmatinv;
		delete[] negcovmatinv;
		delete[] transformvec;
...
struct WinDetectClassify : public WinDetect{
    ...
    WinDetectClassify() :
        ...
        aligninimage(false), showscore(true), use_bias(false),
    ...
    void  test(const LinearClassify& classifier, std::list<DetectedRegion>& detections,
            const unsigned char* imagedata, int width, int height, int step=0, bool hardtest=false) const;
    ...
    void runImageSlider(
        ...
        const std::string& testlocs, bool hardtest=false 
        )  const;
#endif
