...
//      }
	...
	/// Increment just one bin of histogram corresponding to value v
	inline HistElementType& addbin(const ValueType v, const HistElementType e) {
		return (histogram_(v) += e);
    }
    /// Increment just one bin of histogram corresponding to value v
    inline HistElementType addbin(const ValueType v, const HistElementType e) const {
		return (histogram_(v) += e);
    }

	/*/// Compute absolute value of a bin
	inline HistElementType& fabsolute1(const ValueType v) {
		return (fabs(histogram_(v)));
    }
    /// Compute absolute value of a bin
    inline HistElementType fabsolute1(const ValueType v) const {
		return (fabs(histogram_(v)));
    }*/
	...
	/// set all the bins to have positive values
    inline ImplementorType& fabsolute() { 
        histogram_=fabs(histogram_);
        return static_cast<ImplementorType&>(*this);
    //    histogram_ = fabs(histogram_); 
    }
