...
{ //{{{
    ...
    DescNormalizer<RealType>* normalizer1=NULL;
	switch (param.norm2use_exhog) {
        case RHOGDenseParam::NoNorm:
            normalizer1 = new DescNormalizer<RealType>();
            break;

        case RHOGDenseParam::NormL1:
            normalizer1 = new L1Normalizer<RealType>(param.epsilon1);
            break;

        case RHOGDenseParam::NormL2:
            normalizer1 = new L2Normalizer<RealType>(param.epsilon1);
            break;

        case RHOGDenseParam::NormL2Hys:
            normalizer1 = new L2HysNormalizer<RealType>(param.epsilon1, param.maxvalue1);
            break;

        case RHOGDenseParam::NormL1Sqrt:
        default:
            normalizer1 = new L1SqrtNormalizer<RealType>(param.epsilon1);
            break;
    }
    ...
    WinDescType::DescType* desc = new RHOGDense(
                ...
                preprocessor, normalizer, normalizer1);// owns preprocessor & normalizer and frees them when they not required
...
class JitterWindow {// {{{
            ...
            const IndexType startloc) 
    {
        ...
        for (blitz::DomainIter<2> i(-steps_,steps_); i; ++i) {
            ...
            IndexType end ( start+winextent_-1);
...
{// {{{
	...
	float descsize1 = (float)descsize/1024.0;

        maxdescriptor=memorylimit*1024/descsize1;
        if (poscases)
		...
		if (verbose > 1) {
            cout<< "Total lines are " << totallines 
                ...
                << ", maximum descriptors are " << maxdescriptor << endl;
        }
    }
    ...
    samples = count/maxdescriptor;
    //if (samples==1)
	//samples -= 1;
    cout << "Total samples > hardscore " << count << endl; 
    cout << "Samples value is " << samples << endl; 
    
    std::ifstream from1(hardfile.c_str());
    if (!from1) {
        throw lear::Exception("WinDetectDump::writeHardTest()",
                "Unable to open hard test case file " + hardfile);
    }
    ...
    while (from1 ) {
        ...
        IndexType lbound, extent;
        RealType scale, score;
        from1 >> lbound[0] >> lbound[1] >> extent[0] >> extent[1] 
             >> scale >> score;

	if (count == maxdescriptor)
	    continue;
...
{// {{{
            ...
            f!= inlist.end();++f)
    {
		...
		ex = pyimg.extent() - winsize;

		RandEng randX(rng, boost::uniform_int<>(topleft[0]+1,ex[0]));
		RandEng randY(rng, boost::uniform_int<>(topleft[1]+1,ex[1]));

		int ex_min;
		ex_min = min(ex);
		if (ex_min < samples){
			for (int i= 0; i< ex_min; ++i){
				IndexType tl;
				tl[0] = randX(); 
				tl[1] = randY(); 
	...
	const bool hardtest)
{
    ...
    return lear::DetectInfo(
            ...
            blitz::ceil(c - s/2), blitz::floor(s), lbound, hardtest);
}
...
void WinDetectClassify::test(
    ...
    const unsigned char* imagedata, int width, int height, int step, bool hardtest 
    ) const
{//{{{
    if (!descholder.initialized) {
    ...
    {// {{{
                ...
                siter != slider.end(); ++siter) 
        {
            ...
            DetectInfo r = bound(
                    classifier(desc.data(),use_bias),piter.scale(),
                    tl, windesc->extent(),hardtest);
            r.lbound -=toadd;
...
void WinDetectClassify::runImageSlider(
        ...
        const std::string& testlocs, const bool hardtest
        )  const
{ // {{{ 
    using namespace lear;
    using std::setw; using std::setprecision;
    ...
    if (outhist.empty() || (doImageOut && aligninimage)) {
	    ...
	    new Hist_ProcessResult(outhist,lightthreshold, label!='P', -12000,24000,0.1));
    }
    if (doFalseOut) {
        try {
    ...
    holder.push2last(new List_PPResult(outfile, list_marker, hardtest));
    if (!hardtest){
	for (PathVector::const_iterator f=inlist.begin();
            f!= inlist.end();++f){
		string file_list = *f;
		...
		string file_list1 = outfile;
		file_list1.append(file_list);
    	
    		string file_list2 = file_list1;
		it = file_list2.end() - 10;
		file_list2.erase(it,file_list2.end());
		path p(file_list2);
		if(!exists(p))
			create_directories(p);
				 
		fstream filestr;
		filestr.open(file_list1.c_str(),fstream::app);
		filestr.close();
	}
    }
            ...
            f!= inlist.end();++f)
    {
        ...
        if (nopyramid) {
            ...
            DetectInfo r = bound(classifier(desc.data(),use_bias),1, tl, windesc->extent(),hardtest);
			//printf("Score: %f\n",r.score);
			avg += r.score;
...
<< std::endl;
	...
	avg /= (double)totalwindows;
	printf("Average Score: %f\n", avg);
	if (nopyramid)
		printf("Minimum Positive Score: %f\n", min_score);
	else
		printf("Maximum Negative Score: %f\n", max_score);
    if (verbose > 0)  {// {{{
...
LinearClassify::LinearClassify(std::string& modelfile, const int verbose) 
    :
    actdim(0),totwords(0),posavg(0),negavg(0),poscovmatinv(0),negcovmatinv(0),transformvec(0),bias(0)
{// {{{
    	FILE *file;
	if ((file = fopen (modelfile.c_str(), "rb")) == NULL)
		{ throw Exception("LinearClassify::LinearClassify", "Unable to open the classifer file"); }

	fread(&actdim, sizeof(int),1,file);
	fread(&totwords, sizeof(int),1,file);
	
	posavg = new double[totwords];
	negavg = new double[totwords];
	poscovmatinv = new double*[totwords];
	negcovmatinv = new double*[totwords];
	transformvec = new double*[totwords];
	for (int i=totwords;i--;){
		poscovmatinv[i] = new double[totwords];
		negcovmatinv[i] = new double[totwords];
		transformvec[i] = new double[actdim];
        }

	for (int i=0;i<totwords;i++){
		fread(&(posavg[i]), sizeof(double),1,file);
		fread(&(negavg[i]), sizeof(double),1,file);
		for (int j=0;j<totwords;j++){
			fread(&(poscovmatinv[i][j]), sizeof(double),1,file);
			fread(&(negcovmatinv[i][j]), sizeof(double),1,file);
		}
		for (int j=0;j<actdim;j++){
			fread(&(transformvec[i][j]), sizeof(double),1,file);
		}
	}
	fread(&bias,sizeof(double),1,file);
	cout << "The default threshold is " << bias << endl;
	fclose(file);
...
double LinearClassify::operator()(const double* desc, const bool use_bias) const 
{
	double sum1,sum2,score,score1;
	double* descn = new double[totwords];
	double* descp = new double[totwords];
	
	for (int i=totwords;i--;){
		sum1 = 0;
		for (int j=actdim;j--;)
			sum1 += transformvec[i][j]*desc[j];
		descn[i] = sum1 - negavg[i];
		descp[i] = sum1 - posavg[i];
	}
	...
	for (int i=totwords;i--;){
		sum1 = 0;
		sum2 = 0;
		for (int j=totwords;j--;){
			sum1 += descn[j]*negcovmatinv[j][i];
			sum2 += descp[j]*poscovmatinv[j][i];
		}
		score += sum1*descn[i];
		score1 += sum2*descp[i];	
	}
	...
	if (use_bias)
		return (score-score1-bias);
	else
		return (score-score1);
}

float LinearClassify::operator()(const float* desc, const bool use_bias) const 
{
    	double sum1,sum2,score,score1;
	double* descn = new double[totwords];
	double* descp = new double[totwords];
...
{// {{{
    ...
    //totwords = PERSON_WEIGHT_VEC_LENGTH;
    //linearwt_ = new double[totwords];
