...
"  Use iff working with parts & top-left is valid")
        ...
        ("use_bias",bool_option(&(param->use_bias)),
            "if true, use bias to subtract from score after Mahalanobis distance computation")
    ...
    cmdline.addArgument()
        ("modelfile",option<std::string>(&modelfile),
            "learned model file. NOTE: If value is equal to 'defaultperson', it loads default person detector.")
	;
}// }}}
...
unsigned RHOGDenseMain::size() const
{
    ...
    svec.push_back(normalizer1.size());
    ...
    svec.push_back(epsilon1.size());
    svec.push_back(maxvalue.size());
    svec.push_back(maxvalue1.size());
...
void RHOGDenseMain::setRHOGDenseParam(lear::Cmdline& cmdline) 
{
    ...
    cmdline.addOption()
        ...
        ("norm_exhog",option<MultStrOpt>(&(normalizer1))
            ->defaultValue(normalizeropt_.defaultOption()),
            "Descriptor normalizing method for ExHoG to use")
        ...
        ("epsilon",option<MultRealOpt>(&(epsilon))
            ->defaultValue(1)->minValue(0),
            "epsilon to add while normalizing hog")
	("epsilon_exhog",option<MultRealOpt>(&(epsilon1))
            ->defaultValue(1)->minValue(0),
            "epsilon to add while normalizing exhog")
	...
	("maxvalue_exhog",option<MultRealOpt>(&(maxvalue1))
            ->defaultValue(0.2)->minValue(0)->maxValue(1),
            "chop feature vector values for ExHoG > maxvalue after normalizing\n"
    "  Used iff normalizing method == NormL2Hys")
        ;
}
...
std::vector<const RHOGDenseParam*> RHOGDenseMain::fill() const
{
    ...
    for (unsigned i= 0; i< numdesc; ++i) {
		...
		if (normalizer1.size())
            desc->norm2use_exhog = (RHOGDenseParam::NormalizerFlags)normalizeropt_.check(normalizer1.at(i));
		...
		if (epsilon1.size()) 
            desc->epsilon1 = epsilon1.at(i);
        
		if (maxvalue.size()) 
            desc->maxvalue = maxvalue.at(i);

		if (maxvalue1.size()) 
            desc->maxvalue1 = maxvalue1.at(i);
